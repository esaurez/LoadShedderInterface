...
def build_util_matrix(frame_dirs, bin_files, final_bin_size, filter_color, filter_pixel_fraction, training_ratio, max_weight):
...
def compute_combined_util_matrix(util_matrix):
    ...
    return combined_util_matrix

def compute_util_amplification_factor(combined_util_matrix, frame_dirs, bin_files, final_bin_size, filter_color, filter_pixel_fraction, training_ratio, max_weight=None):
    max_util = 0
    for idx in range(len(frame_dirs)):
        bin_file = bin_files[idx]
        frame_dir = frame_dirs[idx]
