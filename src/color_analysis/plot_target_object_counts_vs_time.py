...
def main(bin_dir, outdir):
    bin_files = [join(bin_dir, o) for o in listdir(bin_dir) if isfile(join(bin_dir, o)) and o.endswith(".bin")]
    data = {}
    for b in bin_files:
        observations = python_server.mapping_features.read_samples(b)
        vid = basename(b)
        data[vid] = [[], []]
        frame_id=0
        for observation in observations:
            if observation.label:
                count = observation.detections.totalDetections
            else:
                count = 0
            data[vid][0].append(frame_id)
            data[vid][1].append(count)
            
            frame_id += 1
    
    videos_per_plot = 4
    rows = math.ceil(len(bin_files)/videos_per_plot)
    fig, axs = plt.subplots(nrows=rows, ncols=1, figsize=(12,4))
    vid_idx = 0
    for vid in data:
        ax_idx = int(vid_idx/videos_per_plot)
        ax = axs[ax_idx]

        ax.plot(data[vid][0], data[vid][1], label=vid) 
        ax.legend()

        vid_idx += 1

    fig.savefig(join(outdir, "target_objects.png"))
...
if __name__ == "__main__":
    ...
    parser.add_argument("-B", "--bin-dir", dest="bin_dir", help="Directory containing BIN files")
    parser.add_argument("-O", "--out-dir", dest="output_dir", help="Path to output dir that would contain plots")
    
    args = parser.parse_args()
    main(args.bin_dir, args.output_dir)
